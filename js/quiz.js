// ─────────────────────────────────────────────────────────────
//  QUIZ ENGINE
//  Draws 8 random questions from a bank, randomizes choice order,
//  shows per-choice explanations for wrong answers.
// ─────────────────────────────────────────────────────────────
...
// State
const quizState = {
  crypto: { bank:[], queue:[], idx:0, score:0 },
  arch:   { bank:[], queue:[], idx:0, score:0 },
};
...
function buildQuizQueue(bank) {
  // Shuffle bank, take first QUIZ_SIZE
  return shuffle(bank).slice(0, QUIZ_SIZE).map(q => {
    // Shuffle choices while tracking which one is correct
...
function initQuiz(domain) {
  ...
  s.bank  = domain === 'crypto' ? window.cryptoBank : window.archBank;
...
function renderQuiz(domain) {
  const s   = quizState[domain];
  const pfx = domain === 'crypto' ? 'cs' : 'as';

  // progress bar
  const pct = (s.idx / QUIZ_SIZE) * 100;
  document.getElementById(`${pfx}-progress`).style.width = pct + '%';

  // header
  document.getElementById(`${pfx}-counter`).textContent =
    s.idx < QUIZ_SIZE ? `Question ${s.idx + 1} of ${QUIZ_SIZE}` : 'Complete!';
  document.getElementById(`${pfx}-score-live`).textContent =
    s.idx < QUIZ_SIZE ? `Score: ${s.score}/${s.idx}` : `Final: ${s.score}/${QUIZ_SIZE}`;

  const box = document.getElementById(`${pfx}-box`);

  if (s.idx >= QUIZ_SIZE) {
    renderScoreScreen(domain, pfx, box);
    return;
  }
  ...
  box.innerHTML = `
    ...
    <button class="next-btn" id="${pfx}-next"
      onclick="${s.idx + 1 < QUIZ_SIZE ? `advanceQuiz('${domain}')` : `renderQuiz('${domain}')`}">
...
function handleAnswer(domain, chosenIdx) {
  ...
  // Style buttons + reveal ALL explanations
  q.shuffledChoices.forEach((c, i) => {
    ...
    if (i === q.correctShuffledIdx) {
      ...
      // Show wrong explanations for unchosen wrong answers too
      exp.classList.add('show');
    }
  });
...
function advanceQuiz(domain) {
  quizState[domain].idx++;
  renderQuiz(domain);
}
...
function renderScoreScreen(domain, pfx, box) {
  ...
  if (pct >= 88)      { grade = 'A'; gradeClass = 'grade-A'; msg = 'Excellent — exam-ready on this domain.'; }
  else if (pct >= 75) { grade = 'B'; gradeClass = 'grade-B'; msg = 'Good — review the questions you missed.'; }
  else                { grade = 'C'; gradeClass = 'grade-C'; msg = 'Keep drilling — review explanations carefully.'; }
